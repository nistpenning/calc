...
function [r,xsquare,ysquare,x,y] = pairFullDistance(u,z)
...
zsquare = zeros(N); % N x N pairwise z distance matrix

for i=1:N
    xsquare(i,:) = x(i) - x; % pairwise x distance between particles 
    ysquare(i,:) = y(i) - y; % pairwise y distance between particles 
    zsquare(i,:) = z(i) - z; % pairwise z distance between particles 
end

r = sqrt(xsquare.^2 + ysquare.^2 + zsquare.^2); %square matrix of radial distances
