...
save = False
save_vs_t = True
img_name = "spinNoise_11_03_Adeph"
legend = True
...
h = np.array([True,True,True,True,True,True,False])
...
if True:
    ...
    for i,t in enumerate(int_times):
        ...
        if t == 0.0:
            ...
            Jbar =   J1ks[i]/(0.002/t) #  popt[i][0] #
        S_psi = squ.OAT_fit_func(psis_fit, t, Jbar, Ns[i], G_el_loc,G_ud,G_du, A_d)
        S_psi_up = squ.OAT_fit_func(psis_fit, t, Jbar, Ns[i], G_el_loc,G_ud,G_du, A_d_up)
