...
#Load properties data
    ...
    sig_sub = np.mean(sqrt(pmterr**2 - avg_pmt_counts))
    sig_full = np.mean(pmterr)
    sig_full_err = np.std((pmterr))/sqrt(2*np.size(avg_pmt_counts))
    
    Ns.append(N)  
    sig_PN_subs.append(sig_sub/k)
    sig_PN_fulls.append(sig_full/k)  
    sig_PN_fulls_errs.append(sig_full_err/k)
...
#load calibration data
    ...
    sig_cal_pn = sqrt((np.mean(data.T[2])**2 - cal_counts_avg))
    
    Ns.append(N)  
    sig_PN_subs.append(sig_cal_pn/k)
    sig_PN_fulls.append(sig_cal_ob/k)  
    sig_PN_fulls_errs.append(sig_cal_ob_err/k)
    data_names.append(file_name)
    os.chdir(base_path)
